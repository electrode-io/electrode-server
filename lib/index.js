...
function startElectrodeServer(server) {
  ...
  emitter.emit("server-created", server);
  ...
  return Promise.try(setConnections)
    ...
    .tap((plugins) => { emitter.emit("plugins-sorted", plugins); })
    .then(registerPlugins)
    .tap(emitter.emit("plugins-registered", server))
    .then(startServer)
    .tap(emitter.emit("server-started", server))
    .then(invokeInitializerRefresh)
    .then(invokeCcmRefresh)
    .then(logStarted)
    .then(() => server)
    .tap(emitter.emit("complete", server))
    .catch(startFailed);
}
...
module.exports = function electrodeServer(appConfig, callback) {
  ...
  const makeHapiServerConfig = (serverConfig) => {
    ...
    emitter.emit("config-composed", serverConfig);
