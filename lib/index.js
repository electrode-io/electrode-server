  ...
  const applyDefaultConfigs = (serverConfig) => {
    ...
    if (process.env.NODE_ENV) {
      const envConfigPath = Path.join(
        __dirname,
        "/config/",
        process.env.NODE_ENV.concat(".js")
      );

      try {

        envConfig = require(envConfigPath); // eslint-disable-line global-require

      } catch (e) {
        if (e.code !== "MODULE_NOT_FOUND") {
          // rethrow error if module was found but failed to load
          throw e;
        }
        /*eslint-disable no-console*/
        console.log(`No config file found for environment: ${process.env.NODE_ENV}`);
        /*eslint-enable no-console*/
      }
    }

    Config.util.extendDeep(defaults, envConfig, serverConfig);
    return defaults;
  };
