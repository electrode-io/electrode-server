...
module.exports = function electrodeServer(appConfig, callback) {
  ...
  const applyDefaultConfigs = (serverConfig) => {
    ...
    if (process.env.NODE_ENV) {
      const filename = process.env.NODE_ENV.concat(".js");
      const envConfigPath = Path.join(__dirname, "/config/") + filename;
      ...
      try {
        ...
        /*eslint-disable no-console*/
        console.log("Failed to find environment default config, ignoring.");
        /*eslint-enable no-console*/
      }
    }
