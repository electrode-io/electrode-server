...
module.exports = function electrodeServer(appConfig, callback) {
  ...
  const makeHapiServerConfig = (serverConfig) => {
    ...
    if (serverConfig.server) {
      Hoek.merge(hapiServerConfig, serverConfig.server, true, false);
    }
  ...
  const envConfig = (nodeEnv) => {
    if (nodeEnv) {
      const envConfigPath = require.resolve(Path.join(__dirname, "/config/", nodeEnv));
  ...
  const applyDefaultConfigs = (serverConfig) => {
    ...
    Hoek.merge(defaults, envConfig(process.env.NODE_ENV), true, false);
    Hoek.merge(defaults, serverConfig, true, false);
    return defaults;
  };
