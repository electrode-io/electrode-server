...
function startElectrodeServer(server) {
  ...
  lifecycleData.server = server;
  emitter.emit("server-created", lifecycleData);
  ...
  return Promise.try(setConnections)
    ...
    .tap((plugins) => {
      lifecycleData.plugins = plugins;
      emitter.emit("plugins-sorted", lifecycleData);
    })
    .then(registerPlugins)
    .tap(() => {emitter.emit("plugins-registered", lifecycleData); })
    .then(startServer)
    .tap(() => { emitter.emit("server-started", lifecycleData); })
    .then(invokeInitializerRefresh)
    .then(invokeCcmRefresh)
    .then(logStarted)
    .then(() => server)
    .tap(() => { emitter.emit("complete", lifecycleData); })
    .catch(startFailed);
}
...
module.exports = function electrodeServer(appConfig, callback) {
  ...
  const applyDefaultConfigs = (serverConfig) => {
    ...
    lifecycleData.config = defaults;
    emitter.emit("config-composed", lifecycleData);
