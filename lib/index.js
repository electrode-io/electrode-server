...
function startElectrodeServer(server) {
  ...
  const invokeRefresher = (refreshPath, msg) => {
    return new Promise((resolve, reject) => {
      if (refreshPath) {
      ...
      .catch((err) => {
        console.log(`Electrode Server - ${msg} returned error:`, err); // eslint-disable-line
      });
  };
  ...
  const invokeInitializerRefresh = () => {
    ...
    return invokeRefresher(refreshPath, "service initializer refresh");
  };

  const invokeCcmRefresh = () => {
    const refreshPath = config.services.ccmRefreshPath;

    function invokeCCM() {
      return invokeRefresher(refreshPath, "ccm refresh");
    }

    if (config.ccm) {
      const interval = config.ccm.interval || FIVE_MIN;
      setInterval(invokeCCM, interval);
    }
