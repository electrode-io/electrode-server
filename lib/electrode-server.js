...
function showRegisterPluginsNote(timeout) {
  const noteDelay = Math.ceil((timeout >= 10 * 1000) ? (timeout / 5) : 2 * 1000); // eslint-disable-line
...
function startElectrodeServer(context) {
  ...
  const registerPlugins = (plugins) => {
    ...
    const timeout = Math.max(3000, config.electrode.registerPluginsTimeout); // eslint-disable-line
    ...
    return (new Promise((resolve, reject) => {
      ...
      .timeout(timeout)
      .tap(() => clearTimeout(noteTimeout))
      .catch(Promise.TimeoutError, () => {
        return new Promise((resolve, reject) => {
          server.stop(() => {
            reject(new Error("Electrode Server register plugins timeout.  Did you forget next() in a plugin?")); // eslint-disable-line
          });
        });
      });
  };
  ...
  const logStarted = () => {
    if (context.config.electrode.suppressStartupBanner) {
      return;
    }
...
module.exports = function electrodeServer(appConfig, decors, callback) {
  ...
  const start = (context) =>
    Promise.try(() => new Hapi.Server(makeHapiServerConfig(context)))
      .then((server) => {
