    ...
    const doRequire = () => {
      return (
        ...
        Promise.resolve(fullRequirePath(name))
          ...
          .then(mod => {
            return (p.plugin = mod.default ? mod.default : mod);
          })
      );
    };

    return Promise.try(doRequire).then(() => {
      return p;
    });
  };
...
function startElectrodeServer(context) {
  ...
  const registerPlugins = async plugins => {
    ...
    const awaitTimeout = timeoutMs => new Promise(resolve => resolve).timeout(timeoutMs);
    try {
      await Promise.each(plugins, async plg => {
        try {
          await Promise.race([server.register(plg), awaitTimeout(timeout)]);
        } catch (err) {
