...
double check your plugins to make sure each one resolves its promises.
...
function convertPluginsToArray(plugins) {
  ...
  const loadModule = p => {
    ...
    const doRequire = () => {
      ...
      if (pluginMod.requireFromPath) {
        fromPath = ` from path: ${pluginMod.requireFromPath}`;
        name = pluginMod.name;
        // use require-at to load the module from path
        xrequire = requireAt(pluginMod.requireFromPath);
      } else {
      ...
      return Promise.try(() => xrequire(name))
        ...
        .then(mod => {
          p.plugin = mod.default ? mod.default : mod;
          return p;
        });
    };
...
function startElectrodeServer(context) {
  ...
  const registerPlugins = plugins => {
    const failMsg = (plg, msg) => {
      let withInfo;
      if (plg.module) {
        withInfo = `with module '${JSON.stringify(plg.module)}'`;
      } else {
    ...
    return Promise.each(plugins, plg => {
      return Promise.resolve(server.register(plg))
        .timeout(timeout)
        .catch(err => {
          if (err instanceof Promise.TimeoutError) {
            return server.stop().then(() => {
              throw new Error(failMsg(plg, "timeout - did you return a resolved promise?"));
            });
          } else {
