...
function showRegisterPluginsNote(registerState, timeout) {
    const noteDelay = Math.ceil(timeout >= 10 * MS_PER_SEC ? timeout / 5 : Math.min(3000, timeout / 2)); // eslint-disable-line
...
function convertPluginsToArray(plugins) {
    ...
    const loadModule = p => {
        ...
        const getPluginModule = () => {
            ...
            if (_.isString(p.module)) {
                ...
                assert(!p.module.requireFromPath || _.isString(p.module.requireFromPath), `plugin ${plgKey} 'module.requireFromPath' must be a string`);
        ...
        const doRequire = () => {
            ...
            return xaa
                ...
                // - mod.hapi17Plugin
                // - mod.default.hapi17Plugin (ES6)
                // - mod.hapiPlugin
                // - mod.default.hapiPlugin (ES6)
                // - mod.plugin
                // - mod.default.plugin (ES6)
                // - mod.default (ES6)
                // - mod itself
                const pluginField = [
                    "hapi17Plugin",
    ...
    return xaa.wrap(pluginsArray).then((arr: any) => xaa.map(arr, loadModule));
}
function startElectrodeServer(context) {
    ...
    const registerPlugins = plugins => {
        ...
        return xaa
            .each(plugins, plg => {
            (registerState as any).plugin = plg;
...
export = function electrodeServer(appConfig = {}, decors, callback) {
    const check = () => {
        checkNodeEnv();
    ...
    check();
    const makeHttp2Listener = config => {
    ...
    const makeHapiServerConfig = context => {
        ...
        if (context.config.http2 && context.config.http2.enable) {
            (hapiServerConfig as any).listener = makeHttp2Listener(context.config.http2);
        }
        //
        // This will allow Hapi to make config available through
        // server.settings.app.config
        //
        (hapiServerConfig.app as any).config = context.config;
        return hapiServerConfig;
    };
    const start = context => xaa
        ...
        server.listener.keepAliveTimeout = _.get(context, "config.electrode.keepAliveTimeout", DEFAULT_KEEPALIVE_TIMEOUT);
        // Start server
